...
cp -r elasticsearch-7.5.1 /opt/es/elasticsearch-7.5.1-node1

vim /opt/es/elasticsearch-7.5.1-node1/config/elasticsearch.yml
修改如下配置
...
cp -r elasticsearch-7.5.1 /opt/es/elasticsearch-7.5.1-node2
cp -r elasticsearch-7.5.1 /opt/es/elasticsearch-7.5.1-node3
vim /opt/es/elasticsearch-7.5.1-node2/config/elasticsearch.yml
...
vim /opt/es/elasticsearch-7.5.1-node3/config/elasticsearch.yml
cluster.name: hcq
node.name: cq_node_3
network.host: 0.0.0.0
http.port: 9202
...
discovery.zen.ping.unicast.hosts: ["127.0.0.1:9300", "127.0.0.1:9301","127.0.0.1:9302"]
```
需要修改es文件夹启动的用户,因为es不允许root用户启动.
su root
cd /opt
sudo chown -R hcq:users es
...
windows和linux下都相同,选择在linux下安装
```shell
tar -xzvf kibana-7.5.1-linux-x86_64.tar.gz 
es mv kibana-7.5.1-linux-x86_64 /opt/es/kubana-7.5.1
es cd /opt/es/kubana-7.5.1/config

修改如下
server.port: 5601
server.host: 192.168.1.106 # 此处如设置成localhost其他机器访问不了
elasticsearch.hosts: ["http://localhost:9200"] # 可设置多个 为es的服务地址
...
须花费时间学习
例如mysql收集到logstash
logstash5版本以上已集成jdbc插件，无需下载安装，直接使用
```shell
input {
    jdbc {
      # mysql 数据库链接,shop为数据库名
      jdbc_connection_string => "jdbc:mysql://localhost:3001/test"
      # 用户名和密码
      jdbc_user => "root"
      jdbc_password => "root"
      # 驱动位置
...
有很多这类的配置文件,logstash有许多beats可以从不同的端收集数据到es中.
本次只是安装而已,学习使用是一个还需花费时间.